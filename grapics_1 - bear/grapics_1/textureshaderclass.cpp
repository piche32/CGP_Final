...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd) 
{
	bool result;
	// 버텍스, 픽셀 쉐이더를 초기화합니다.
	result = InitializeShader(device, hwnd, L"../grapics_1/VertexShader.hlsl", L"../grapics_1/PixelShader.hlsl"); 
	if(!result) 
	{ 
		return false; 
	} 
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Create the constant buffer pointer so we can access the verex shader constant buffer from within this class.
	...
	samplerDesc.MipLODBias = 0.0f; samplerDesc.MaxAnisotropy = 0;
...
void TextureShaderClass::ChangeTexture() {
	D3D11_SAMPLER_DESC samplerDesc;

	samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
	samplerDesc.MipLODBias = 0.0f; samplerDesc.MaxAnisotropy = 1;
	samplerDesc.ComparisonFunc = D3D11_COMPARISON_ALWAYS;
	samplerDesc.BorderColor[0] = 0; samplerDesc.BorderColor[1] = 0;
	samplerDesc.BorderColor[2] = 0; samplerDesc.BorderColor[3] = 0;
	samplerDesc.MinLOD = 0; samplerDesc.MaxLOD = D3D11_FLOAT32_MAX;
	...
	m_hi->CreateSamplerState(&samplerDesc, &m_sampleState);
